...
def two_stage(gal, psf, rng_noise, rng_metacal, h=0.01, n_mu=0.0, n_sigma_wide=1e-4, n_sigma_deep=1e-6, weight_fwhm=1.2):
    ...
    for _ in range(10):
        ...
        obs_deep.image = rng_noise.normal(n_mu, n_sigma_deep, (53, 53))
        ...
        vs.append(np.std(noshear_deep.image)**2)
    
    n_sigma_deep2wide = np.sqrt(n_sigma_wide**2 - np.mean(vs))
    noise_deep2wide = rng_noise.normal(n_mu, n_sigma_deep2wide, (53, 53))
    
    obs_deep = sim_func(gal=gal, psf=psf, g1=0.0, g2=0.0, noise=noise_deep, n_sigma=n_sigma_deep)
    obs_wide = sim_func(gal=gal, psf=psf, g1=0.0, g2=0.0, noise=noise_wide, n_sigma=n_sigma_wide) # compare to this
    metacal_ims = ngmix.metacal.get_all_metacal(obs_deep, psf='fitgauss', fixnoise=True,
                                               step=h, rng=rng_metacal)
    ...
    noshear_wide = add_noise(noshear_deep, noise_deep2wide, n_sigma_deep2wide=n_sigma_deep2wide) # measure corr in this
    shearg1p_wide = add_noise(shearg1p_deep, noise_deep2wide, n_sigma_deep2wide=n_sigma_deep2wide)
    shearg1m_wide = add_noise(shearg1m_deep, noise_deep2wide, n_sigma_deep2wide=n_sigma_deep2wide)
    shearg2p_wide = add_noise(shearg2p_deep, noise_deep2wide, n_sigma_deep2wide=n_sigma_deep2wide)
    shearg2m_wide = add_noise(shearg2m_deep, noise_deep2wide, n_sigma_deep2wide=n_sigma_deep2wide)
    
    from ngmix.ksigmamom import KSigmaMom
    ks = KSigmaMom(1.2)
    noshear_meas = ks.go(obs_wide)
    shearg1p_meas = ks.go(shearg1p_wide)
    shearg1m_meas = ks.go(shearg1m_wide)
    shearg2p_meas = ks.go(shearg2p_wide)
    shearg2m_meas = ks.go(shearg2m_wide)
    ...
    t_noshear = noshear_meas['T']/noshear_meas['T_err']
    t_g1p = shearg1p_meas['T']/shearg1p_meas['T_err']
    t_g1m = shearg1m_meas['T']/shearg1m_meas['T_err']
    t_g2p = shearg2p_meas['T']/shearg2p_meas['T_err']
    t_g2m = shearg2m_meas['T']/shearg2m_meas['T_err']
        
    if t_noshear>1.2 and t_g1p>1.2 and t_g1m>1.2 and t_g2p>1.2 and t_g2m>1.2: 
        t_flag = 0
    else: 
        t_flag = 1
...
for i in tqdm.trange(0,n):
    rng_noise = np.random.RandomState(np.random.randint(0,2**31-1))
    rng_metacal = np.random.RandomState(np.random.randint(0,2**31-1))
    e_p, r_p, flag_p = two_stage(gal_p, psf, rng_noise, rng_metacal, n_sigma_wide=1e-2, n_sigma_deep=3e-3)
    e_m, r_m, flag_m = two_stage(gal_m, psf, rng_noise, rng_metacal,n_sigma_wide=1e-2, n_sigma_deep=3e-3)
    if flag_p==0 and flag_m==0:
        e = [(e_p[0]-e_m[0])/2., (e_p[1]+e_m[1])/2.]
        e_arr.append(e)
        r_arr.append((r_p+r_m)/2.)
e_arr1 = np.array(e_arr)
r_arr1 = np.array(r_arr)